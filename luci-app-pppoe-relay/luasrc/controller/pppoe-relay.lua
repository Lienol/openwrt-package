...
function index()
    if not nixio.fs.access("/etc/config/luci-app-pppoe-relay") then return end
    entry({"admin", "services", "pppoe-relay"}, cbi("pppoe-relay"), _("PPPoE Relay"), 2).dependent = true
    entry({"admin", "services", "pppoe-relay", "get_status"}, call("get_status")).leaf = true
end
...
function get_status()
    ...
    local server_interface = uci:get("luci-app-pppoe-relay", section, "server_interface")
    local client_interface = uci:get("luci-app-pppoe-relay", section, "client_interface")
